...
module.exports = async function moduleReportHandler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      historyUrl.searchParams.set("studentId", studentId);
      ...
      if (!response.ok || !result.ok || !Array.isArray(result.attempts)) {
        return sendJson(res, result.code === "STUDENT_NOT_FOUND" ? 404 : 502, {
          ok: false,
          message: result.message || "Previous module submissions could not be loaded."
        });
      }
      ...
      return sendJson(res, 502, {
        ok: false,
        message: "Could not reach the module report history service."
      });
    }
  }
  ...
  const reportPayload = {
    ...
    attemptTimestamp: String(payload.attemptTimestamp || ""),
